"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.generateResolvers = void 0;
var auxiliaries_1 = require("../../src/Common/Utils/auxiliaries");
var fs_1 = require("fs");
var generateResolvers = function (path, tableName, oneToMany, manyToOne) { return __awaiter(void 0, void 0, void 0, function () {
    var className, resolverPath, _a, file, mutations;
    return __generator(this, function (_b) {
        className = (0, auxiliaries_1.removeDoctive)(tableName);
        className = (0, auxiliaries_1.snakeToCamelCase)(className);
        resolverPath = "Services/resolvers/" + className + ".ts";
        _a = createInitialFile(className), file = _a.file, mutations = _a.mutations;
        oneToMany.map(function (value) {
            file = addRelationFile(value, className, createOneToManyFile, file);
        });
        manyToOne.map(function (value) {
            file = addRelationFile(value, className, createManyToOneFile, file);
        });
        file += "\n    },\n    " + mutations;
        (0, fs_1.writeFileSync)(path + "/" + resolverPath, file);
        addToIndexFile(className);
        (0, fs_1.writeFileSync)(path + "/Services/resolvers/index.ts", indexFile);
        return [2 /*return*/];
    });
}); };
exports.generateResolvers = generateResolvers;
var addRelationFile = function (value, className, fileFunction, file) {
    var parentName = (0, auxiliaries_1.removeDoctive)(value.foreign_table_name);
    parentName = (0, auxiliaries_1.snakeToCamelCase)(parentName);
    if (className !== parentName) {
        file = "import {" + parentName + "} from '@Common/Entities/" + parentName + "';\n" + file;
    }
    var relationFile = fileFunction(className, parentName);
    file += "\n        " + relationFile;
    return file;
};
var createInitialFile = function (className) {
    var file = "import {" + className + "} from '@Common/Entities/" + className + "';\nimport {CommandFactory} from '@Logic/Commands/CommandFactory';\nimport {GraphQLMutation, GraphQLQuery} from '../graphQLTypes';\nexport const " + className + "Resolver = {\n    Query: {\n        get" + className + "s: async (parent: any, args: GraphQLQuery) => {\n            console.info('get" + className + "s parent:', parent, 'args: ',args);\n            const where = new " + className + "(args.where)\n            const command = CommandFactory.createGet" + className + "sCommand(where, args.limit, args.skip);\n            return command.execute();\n        }\n    },\n    " + className + ": {";
    var mutations = "Mutation: {\n        create" + className + ": async (parent: any, args: GraphQLMutation) => {\n            console.info('create" + className + " parent: ', parent, 'args: ',args);\n            const createData = new " + className + "(args.insertData);\n            const command = CommandFactory.createCreate" + className + "Command(createData);\n            return command.execute();\n        },\n        update" + className + ": async (parent: any, args: GraphQLMutation) => {\n            console.info('update" + className + " parent: ', parent, 'args: ',args);\n            const where = new " + className + "(args.where);\n            const updateData = new " + className + "(args.updateData)\n            const command = CommandFactory.createUpdate" + className + "Command(where, updateData);\n            return command.execute();\n        },\n        delete" + className + ": async (parent: any, args: GraphQLMutation) => {\n            console.info('delete" + className + " parent: ', parent, 'args: ',args);\n            const deleteData = new " + className + "(args.deleteData);\n            const command = CommandFactory.createDelete" + className + "Command(deleteData);\n            return command.execute();\n        }\n    }\n}\n";
    return { file: file, mutations: mutations };
};
var createManyToOneFile = function (className, relationName) {
    var variableName = className === relationName ? "Parent" + relationName : relationName;
    var parentName = className === relationName ? "Sub" + relationName : className;
    return (0, auxiliaries_1.minimizeFirstLetter)(variableName) + ": async (parent: " + className + ", args: GraphQLQuery) => {\n            console.info('" + (0, auxiliaries_1.minimizeFirstLetter)(variableName) + " parent: ', parent, 'args: ',args)\n            const where = new " + relationName + "(args.where);\n            const command = CommandFactory.createGet" + variableName + "By" + parentName + "Command(where, parent);\n            return command.execute();\n        },";
};
var createOneToManyFile = function (className, relationName) {
    var variableName = className === relationName ? "Sub" + relationName : relationName;
    var parentName = className === relationName ? "Parent" + relationName : className;
    return (0, auxiliaries_1.minimizeFirstLetter)(variableName) + "s: async (parent: " + className + ", args: GraphQLQuery) => {\n            console.info('" + (0, auxiliaries_1.minimizeFirstLetter)(variableName) + "s parent: ', parent, 'args: ',args)\n            const where = new " + relationName + "(args.where);\n            const command = CommandFactory.createGet" + variableName + "sBy" + parentName + "Command(where, parent, args.limit);\n            return command.execute();\n        },";
};
var addToIndexFile = function (className) {
    indexFile = indexFile.replace('\n\nexport', "\nimport {" + className + "Resolver} from './" + className + "';\nexport");
    indexFile = indexFile.slice(0, indexFile.length - 3);
    indexFile += !first ? ",\n    " + className + "Resolver\n])" : "    " + className + "Resolver\n])";
    first = false;
};
var indexFile = "import {mergeResolvers} from \"@graphql-tools/merge\";\nexport const resolvers = mergeResolvers([\n])\n";
var first = true;
